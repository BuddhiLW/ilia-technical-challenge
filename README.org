...
*** My solution
I will do the following: instead of a order be an object with the entries ="order_details"= and ="id"=, as shown following.
...
The new data representation will be as an object with =key= being the =string of the value of the id-key=. And the =value= being the original representation of the =object=. As seen following.
...
Thus, we can represent all orders as an object, in which each =key-value= is unique and represent an *order*. Following, the *search* and *insertion* in this data structure is =O(ln(n))=.
